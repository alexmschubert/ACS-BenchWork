...
if tuple(map(int, torch.__version__.split('.')[:2])) >= (1, 10): 
    def _resolve_conj(x): return x.conj().resolve_conj()
else:
...
def rank_correction(measure, N, rank=1, dtype=torch.float):
    ...
    d = P.size(0)
    if rank > d:
        P = torch.cat([P, torch.zeros(rank-d, N, dtype=dtype)],
                      dim=0)  # (rank N)
    return P
...
class SSKernelNPLR(nn.Module):
    ...
    def _setup_linear(self):
        ...
        R = (torch.eye(self.rank, dtype=w.dtype, device=w.device) + 2 *
             contract('r h n, h n, s h n -> h r s', Q, D, P).real)  # (H r r)
    ...
    def _step_state_linear(self, u=None, state=None):
        ...
        if state.size(-1) == self.N:
            ...
            def contract_fn(p, x, y): return contract('r h n, r h m, ... h m -> ... h n',
                                                      _conj(p), _conj(x), _conj(y))[..., :self.N]  # inner outer product
        else:
            assert state.size(-1) == 2*self.N
            step_params = {k: _conj(v) for k, v in step_params.items()}
            # TODO worth setting up a contract_expression in default_state if we want to use this at inference time for stepping
            def contract_fn(p, x, y): return contract(
                'r h n, r h m, ... h m -> ... h n', p, x, y)  # inner outer product
        D = step_params["D"]  # (H N)
        E = step_params["E"]  # (H N)
    ...
    def _setup_state(self):
        ...
        state = torch.eye(2*self.N, dtype=C.dtype,
                          device=C.device).unsqueeze(-2)  # (N 1 N)
    ...
    def _step_state(self, u, state):
        ...
        next_state = self.state_contraction(
            self.dA, state) + self.input_contraction(self.dB, u)
        return next_state
    ...
    def setup_step(self, mode='dense'):
        ...
        if mode == 'linear':
            ...
            if self.verbose:
                print("Diagonalization error:", torch.dist(
                    V @ torch.diag_embed(L) @ V_inv, self.dA))
