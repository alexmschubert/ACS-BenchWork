...
def process_ecg_file(ecg_file, output_dir):
    ...
    try:
        ...
        for lead_idx in range(12):
            ...
            # Corresponding time vector for valid data
            time_no_nan = time_vector[valid_indices]
            ...
            r_peaks_adjusted = r_peaks_valid - min_valid_index  # Adjust indices to match lead_data_no_nan
            
            # Check if there are enough R-peaks
            if len(r_peaks_adjusted) < 1:
                continue  # Skip if not enough R-peaks
            ...
            if len(beats) == 0:
                continue  # Skip if no beats extracted
            ...
            # Normalize the extended lead to prevent amplitude increase due to overlaps
            # max_abs = np.max(np.abs(extended_lead))
            # if max_abs != 0:
            #     extended_lead = extended_lead / max_abs
            
            # Store the processed lead
            processed_leads[lead_idx] = extended_lead
        
        # Handle the last 3 leads (indices 12, 13, 14)
        last_3_leads = data[12:15]
        lead_locations = [6, 1, 10]  # Mapping last 3 leads to specific positions
...
def main():
    ...
    output_dir = '/home/ngsci/project/NEJM_benchmark/waveforms_12lead_10sec/'
    ...
    # Optionally, check for any failures
    ...
    # with multiprocessing.Pool(num_workers) as pool:
    #     pool.map(process_func, ecg_files)
